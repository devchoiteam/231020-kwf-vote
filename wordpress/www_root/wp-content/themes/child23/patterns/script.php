...
<script>
    ...
    // // 2차 투표 > 팀선택 > 1팀 이상의 팀을 선택하는 경우
    // 'Sorry, you can select up to 1 teams.'
    ...
    // 투표 시간 이전에 접속: Voting is possible from 7pm.
    // 투표 시간 이후에 접속: Voting has closed with much support. Thank you.
    ...
    // 일정 시간동안만 오픈
    let now = new Date();
    let openTime = new Date('2023-10-27 19:00:00');
    let closeTime = new Date('2023-10-27 23:00:00');
    
    if(openTime > now) {
        // alert('Voting is possible from 7pm.');
        document.querySelector('.voteTime').classList.remove('open');
    }
    else if(now > closeTime) {
        // alert('Voting has closed with much support. Thank you.');
        document.querySelector('.voteTime').classList.remove('open');
        document.querySelector('.voteTime').classList.add('closed');
    }
